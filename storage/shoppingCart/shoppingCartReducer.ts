import { ADD_ITEM_TO_SHOPPING_CART, ADD_ALL_ITEMS_TO_SHOPPING_CART, REMOVE_ITEM_FROM_SHOPPING_CART, REMOVE_ALL_ITEMS_FROM_SHOPPING_CART, EDIT_SHOPPING_CART_ITEM, shoppingCartInitState } from "./shoppingCartConstants"

import { ShoppingCartActionTypes, ShoppingCart, ShoppingCartProduct } from "./shoppingCartTypes";


export const shoppingCartReducer = (state = shoppingCartInitState, action: ShoppingCartActionTypes): ShoppingCart => {
  switch (action.type) {
    case ADD_ITEM_TO_SHOPPING_CART:
      ...
      if (indexOfProduct !== -1) {
        ...
        if (product.Size === action.payload.item.Size &&
          ...
          const changedProduct: ShoppingCartProduct = { ...product };
