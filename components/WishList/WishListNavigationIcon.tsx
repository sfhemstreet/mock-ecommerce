...
const WishListContainer = styled.div<{ willShake: boolean }>`
  cursor: pointer;
  position: relative;
...
export const WishListNavigationIcon = (): JSX.Element => {
  ...
  if (!wishList.data) return <SpinningLoader reverse />
  
  return (
    <WishListContainer tabIndex={0} willShake={wishList.data.products.length > 0}>
      <WishListSVG
